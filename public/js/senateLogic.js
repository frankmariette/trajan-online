...
Senate.prototype.playerVPGain = function(currentPlayer) {
  ...
  this.senateSpaces(currentPlayer.position.x, currentPlayer.position.y);

}

Senate.prototype.movePlayerSenatePiece = function(playerTile, newSpace) {
  playerTile.position.x = newSpace.x;
  playerTile.position.y = newSpace.y;
}
...
Senate.prototype.senateSpaces = function(x,y){
  ...
  for (var i = 0; i < this.senate_spaces.length; i++) {
    if (this.senate_spaces[i].contains(x,y)) {
      this.movePlayerSenatePiece({"x":x,"y":y}, senate_spaces[i+1]);
